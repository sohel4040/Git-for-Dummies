...
pathUsed = 'C:/Users/Rich/Desktop/RawDocuments/Attendace_management_system/'
    ...
    def fill_attendance():
        ...
        if subb=='':
            ...
            def create_csv():
                ...
                csv_name=pathUsed+'Attendance/Manually Attendance/'+DB_table_name+'.csv'
            ...
            def open_folder():
                  path = pathUsed + 'Attendance/Manually Attendance'
                #   folder_path = r'path'
                  os.startfile(path)
            attf = tk.Button(MFW,  text="Check ",command=open_folder,fg="black"  ,bg="lawn green"  ,width=12  ,height=1 ,activebackground = "Red" ,font=('times', 14, ' bold '))
...
def subjectchoose():
    ...
    def open_folder():
                    path = pathUsed + '\Attendance'
                    os.startfile(path)
    attf = tk.Button(windo,  text="Check Sheets",command=open_folder,fg="black"  ,bg="lawn green"  ,width=12  ,height=1 ,activebackground = "Red" ,font=('times', 14, ' bold '))
    ...
    def log_in():
        ...
        if username == 'admin' :
            if password == 'admin':
                ...
                import tkinter
                root = tkinter.Tk()
                root.title("Student Details changed")
                root.configure(background='snow')

                cs = pathUsed + 'StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
